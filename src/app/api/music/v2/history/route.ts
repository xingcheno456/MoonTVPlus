...
function toHistoryRecord(
  ...
  return {
    ...
    playProgressSec: Number(
      input.playProgressSec ??
        input.play_progress_sec ??
        previous?.playProgressSec ??
        0,
    ),
    lastPlayedAt: Number(input.lastPlayedAt ?? input.last_played_at ?? now),
    playCount: Number(
      input.playCount ?? input.play_count ?? (previous?.playCount || 0) + 1,
    ),
    lastQuality:
      input.lastQuality || input.last_quality || previous?.lastQuality,
    createdAt: Number(
      input.createdAt ?? input.created_at ?? previous?.createdAt ?? now,
    ),
    updatedAt: now,
  };
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const existingMap = new Map(
      existingRecords.map((record) => [record.songId, record]),
    );
    ...
    if (Array.isArray(body.records)) {
      const records = body.records
        .map((item: any) =>
          toHistoryRecord(
            item,
            existingMap.get(item.song?.songId || item.songId),
          ),
        )
        .filter(
          (item: MusicV2HistoryRecord) =>
