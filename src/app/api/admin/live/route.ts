    ...
    switch (action) {
      case 'add':
        ...
        if (config.LiveConfig.some((l) => l.key === key)) {
          return apiError('直播源 key 已存在', 400);
        }
      ...
      case 'edit':
        ...
        if (editSource.from === 'config') {
          return apiError('不能编辑配置文件中的直播源', 400);
        }
