...
export default function DanmakuPanel({
  ...
  const handleEpisodeGroupWheel = useCallback(
    (e: WheelEvent) => {
  ...
  useEffect(() => {
    if (isEpisodeGroupHovered) {
      ...
      document.addEventListener('wheel', handleEpisodeGroupWheel, {
        passive: false,
      });
    } else {
  ...
  return (
    <div className='flex h-full flex-col overflow-hidden'>
      ...
      <div className='mb-4 flex-shrink-0'>
        <div className='flex flex-wrap gap-2'>
          <input
            ...
            data-lpignore='true'
            className='min-w-[220px] flex-1 rounded-lg border border-gray-300 px-3 py-2 text-sm transition-colors focus:border-green-500 focus:outline-none focus:ring-2 focus:ring-green-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-white sm:px-4'
            disabled={isSearching}
          />
          <button
            onClick={() => handleSearch(searchKeyword)}
            disabled={isSearching}
            className='flex min-w-[44px] flex-shrink-0 items-center justify-center gap-2 rounded-lg bg-green-500 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-green-600 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-green-600 dark:hover:bg-green-700 lg:px-4'
          >
            {isSearching ? (
        ...
        {/* 错误提示 */}
        {searchError && (
          <div className='mt-3 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-600 dark:text-red-400'>
            {searchError}
          </div>
        )}
      </div>
      ...
      <div className='min-h-0 flex-1 overflow-y-auto'>
        {/* 当前选择的弹幕信息 */}
        {currentSelection && (
          <div className='mb-4 rounded-lg border border-green-500/30 bg-green-500/10 px-3 py-2 text-sm'>
          ...
          {selectedAnime && (
            <div className='space-y-2'>
              ...
              <button
                ...
                className='mb-2 text-sm text-green-600 hover:underline dark:text-green-400'
              >
                ← 返回搜索结果
              </button>
                      ...
                      {currentGroupEpisodes.map((episode) => {
                        ...
                        return (
                          <button
                            key={episode.episodeId}
                            onClick={() => handleEpisodeSelect(episode)}
                            className={`group flex w-full items-center gap-3 rounded-lg border p-3 text-left transition-all duration-200 ${
                              isSelected
                                ? 'border-green-600 bg-green-500 text-white shadow-md'
                                : 'border-gray-200 bg-gray-100 hover:border-green-500/50 hover:bg-gray-200 hover:shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700'
                            }`}
                          >
                            <div
                            ...
                            {isSelected ? (
                              ...
                              <div className='flex-shrink-0'>
                                <svg
                                  className='h-5 w-5 text-gray-400 transition-colors group-hover:text-green-500'
                                  fill='none'
                                  stroke='currentColor'
                                  viewBox='0 0 24 24'
                                >
            ...
            <div className='space-y-2 pb-4'>
              {searchResults.map((anime) => (
                <div
                  ...
                  className='flex cursor-pointer items-start gap-3 rounded-lg bg-gray-100 p-3 transition-colors hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700'
                >
                  {/* 封面 */}
                  {anime.imageUrl && (
                  ...
                  <div className='min-w-0 flex-1'>
                    <div className='relative'>
                      <p className='peer truncate font-semibold text-gray-800 dark:text-white'>
                        {anime.animeTitle}
                      </p>
                      {/* 自定义 tooltip */}
                      <div className='pointer-events-none invisible absolute bottom-full left-1/2 z-[100] mb-2 -translate-x-1/2 transform whitespace-nowrap rounded-md bg-gray-800 px-3 py-1 text-xs text-white opacity-0 shadow-lg transition-all delay-100 duration-200 ease-out peer-hover:visible peer-hover:opacity-100'>
                        {anime.animeTitle}
      ...
      {/* 上传弹幕区域 - PC端：固定在底部 */}
      {onUploadDanmaku && (
        <div className='mt-3 hidden flex-shrink-0 border-t border-gray-200 pt-3 dark:border-gray-700 md:block'>
          <button
            onClick={() => fileInputRef.current?.click()}
            className='w-full py-2 text-center text-xs text-gray-500 transition-colors hover:text-green-500 dark:text-gray-400 dark:hover:text-green-400'
          >
